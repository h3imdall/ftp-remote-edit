...
export default class Connector extends EventEmitter {
  ...
  uploadFile(content, queueItem) {
    ...
    return self.createDirectory(arrPath.join('/'))
      .then(() => {
        return self.connect()
          .then((Client) => {
            return Client.put(content, queueItem)
              .then((remotePath) => {
  ...
  downloadFile(queueItem) {
    ...
    try {
      if (!FileSystem.existsSync(queueItem.info.localPath)) {
        FileSystem.makeTreeSync(queueItem.info.localPath);
      }
    } catch (_error) {}

    return self.connect()
      .then((Client) => {
        return Client.get(queueItem)
          .then((localPath) => {
            queueItem.changeStatus('Finished');
