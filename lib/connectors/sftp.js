...
export default class Sftp extends EventEmitter {
  ...
  dirDFS(self, remotePath, relativeDir = '/') {
    self.client.list(remotePath).then((result) => {
      result.forEach((file) => {
        if(file.type == 'd') {
          self.dirDFS(self, remotePath + file.name + '/', relativeDir + file.name + '/')
        } else {
          filePath = remotePath + file.name;
          relativePath = relativeDir + file.name;
          self.filePaths.push({filePath, relativePath});
        }
      });
    })
  }
