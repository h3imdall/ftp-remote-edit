...
class Storage {
  ...
  load(reload = false) {
    ...
    if (!self.loaded || reload) {
      ...
      if (configText) {
        /*
        if (atom.config.get('ftp-remote-edit.encryption') == 'encrypted') {
          configText = cleanJsonString(decrypt(self.password, configText));
        }
        */

        try {
          configArray = self.migrate(JSON.parse(configText));
        } catch (e) {
          configArray = self.migrate(configText);
        }
  ...
  save() {
    ...
    atom.config.set('ftp-remote-edit.config', configText);
  }
  ...
  migrate(configArray) {
    const self = this;
    let encryptionUpdate = false;

    if(typeof configArray === 'string') {
      // backup current config
      self.backup('0.18.0');
      try {
          configArray = JSON.parse(cleanJsonString(decrypt(self.password, configArray)));
          configArray.servers.forEach((config) => {
            config.password = encrypt(self.password,config.password);
          });
      } catch(e) {
