...
class Storage {
  ...
  constructor() {
    ...
    self.folders = [];
    self.version = '';
    self.state = self.loadDefaultState();
    self.password = null;
    self.loaded = false;
    self.tree = null;
  }
  ...
  load(reload = false) {
    ...
    if (!self.loaded || reload) {
      ...
      if (configText) {
        ...
        try {
          let configArray = self.migrate(JSON.parse(cleanJsonString(configText)));
  ...
  loadState() {
    const self = this;
    const path = Path.dirname(atom.config.getUserConfigPath()) + Path.sep + 'ftp-remote-edit.cson';

    try {
      self.state = JSON.parse(FileSystem.readFileSync(path).toString('utf8'));
    } catch (e) {
      console.log('Loading state failed: ' + path);
      self.state = self.loadDefaultState();
    }
  }
  ...
  saveState() {
    ...
    try {
      FileSystem.writeFileSync(path, JSON.stringify(self.state), { encoding: 'utf8' });
    } catch (e) {
      console.log('Saving state failed: ' + path);
    }
  }
