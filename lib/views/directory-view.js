...
class DirectoryView extends View {
  ...
  isPathIgnored(filePath) {
    ...
    if (atom.config.get('ftp-remote-edit.tree.hideIgnoredNames')) {
      let ignoredPatterns = self.getRoot()
        .treeView
        .ignoredPatterns;
  ...
  paint(list) {
    ...
    directories.forEach(function (element) {
      ...
      if (!self.isPathIgnored(pathOnFileSystem)) {
    ...
    self.getRoot()
      ...
      .catch(function (err) {
        return self.getRoot()
          ...
          .then(() => {
            self.getRoot().updateFinderItemsCache('add', relativePath);
      ...
      .catch(() => {
        ...
        self.getRoot()
          ...
          .then(() => {
            ...
            let arrPath = newObject.getLocalPath()
              .split(Path.sep);
            arrPath.pop();
  ...
  copyFile(initialPath, newDirectoryPath) {
    ...
    let srcLocalPath = (self.getRoot()
        .getLocalPath(true) + initialPath.replace(/^\//, "")
        .replace(/\/+/g, Path.sep))
      .replace(/\/+/g, Path.sep);
    let destLocalPath = (self.getRoot()
        .getLocalPath(true) + newDirectoryPath.replace(/^\//, "")
        .replace(/\/+/g, Path.sep))
      .replace(/\/+/g, Path.sep);
          ...
          .then(() => {
            ...
            FileSystem.stat(srcLocalPath, function (err, stats) {
              if (stats) {
                ...
                // Read content
                if (FileSystem.existsSync(srcLocalPath)) {
                  fileinfo.content = FileSystem.readFileSync(srcLocalPath, 'utf8');
                }
                ...
                return self.getRoot()
                  .connector.uploadFile(fileinfo.content, upload_queueItem)
                  .then(() => {
                    self.getRoot().updateFinderItemsCache('add', dest.substring(self.getRoot().config.remote), self.size);
                    if (!fileinfo.exists) {
                      // Add new object
                      let elementname = basename(dest);
                      let newObject = new FileView(self, {
                    ...
                    try {
                      FileSystem.writeFileSync(destLocalPath, fileinfo.content);
                    } catch (err) {}
                  })
                  .catch(function (err) {
  ...
  initFinderItemsCache(items) {
    const self = this;
    self.finderItemsCache = new FinderItemsCache();
    self.finderItemsCache.setItems(items);
  }

  updateFinderItemsCache(option = 'add', relativePath, size = 0) {
    const self = this;
    ...
    if(option == 'add') {
      self.finderItemsCache.addItem(relativePath, size);
    } else if (option = 'delete') {
      self.finderItemsCache.deleteItem(relativePath);
    }
  }
}
