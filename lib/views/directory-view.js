...
class DirectoryView extends View {
    ...
    self.getRoot()
      ...
      .catch(function (err) {
        return self.getRoot()
          ...
          .then(() => {
            self.updateFinderItemsCache(relativePath, ADD_CACHE);
              ...
              if (stats) {
                ...
                return self.getRoot()
                  ...
                  .then(() => {
                    ...
                    self.updateFinderItemsCache(dest.substring(self.getRoot().config.remote), ADD_CACHE, self.size);
                    if (!fileinfo.exists) {
                      // Add new object
                      let elementname = basename(dest);
                      let newObject = new FileView(self, {
  ...
  updateFinderItemsCache(relativePath, option, size = 0) {
    const self = this;
    let rootDir = self;
    relativePath = relativePath.replace(/^\//, '');
    while(rootDir.parent) {
      rootDir = rootDir.parent
    }
    if(!rootDir.finderItemsCache) {
      return;
    } else {
      if(option == ADD_CACHE) {
        rootDir.finderItemsCache.push({relativePath, size})
      } else if(option == DELETE_CACHE) {
        rootDir.finderItemsCache = rootDir.finderItemsCache.filter((item) => {
