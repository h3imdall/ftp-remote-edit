...
import { getStyleObject } from './../helper/format.js';
...
class DirectoryView extends View {
  ...
  initialize(parent, directory) {
    ...
    self.on('dragover', (e) => self.onDragOver(e));
  ...
  onDragStart(e) {
    if (entry = e.target.closest('.entry')) {
      ...
      let target = entry.querySelector(".name");
      let initialPath = target.dataset.path;
      console.log(target, e);

      let fileNameElement = target.cloneNode(true);
      ref = getStyleObject(target);
      for (key in ref) {
        value = ref[key];
      ...
      if (e.dataTransfer) {
        ...
        e.originalEvent.dataTransfer.setDragImage(fileNameElement, 0, 0);
