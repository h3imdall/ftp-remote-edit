...
class DirectoryView extends View {
  ...
  expand(recursive) {
    ...
    let promise = new Promise((resolve, reject) => {
      ...
      self.connector.loadDirectory(self.getPath())
        .then((list) => {
          self.paint(list);
  ...
  find(relativePath) {
    ...
    let pathArr = relativePath.trim().split('/');
    pathArr.shift();
    let search = pathArr.map(function(item){ return 'li[data-name="' + item + '"]';}).join(" ");
  ...
  newFile(relativePath) {
    ...
    self.connector.saveFileToServer('', relativePath.trim())
      .then(() => {
        self.collapse();
        self.expand().then(() => {
  ...
  delete(recursive) {
    ...
    self.connector.deleteDirectory(self.getPath())
      .then(() => {
        self.destroy();
    });
  }
