...
import { getFullExtension, createLocalPath, isPathIgnored } from './../helper/helper.js';
...
class DirectoryView extends View {
  ...
  paint(list) {
    ...
    // Refresh cache
    self.getRoot().getFinderItemsCache().refreshItems(self.getPath(false), files);
    ...
    self.getRoot()
      ...
      .catch(function (err) {
        return self.getRoot()
          ...
          .then(() => {
            self.getRoot().getFinderItemsCache().addItem(relativePath, 0);
            ...
            FileSystem.stat(srcLocalPath, function (err, stats) {
              if (stats) {
                ...
                return self.getRoot()
                  ...
                  .then(() => {
                    self.getRoot().getFinderItemsCache().addItem(dest.substring(self.getRoot().config.remote), self.size);
                    if (!fileinfo.exists) {
                      // Add new object
                      let elementname = basename(dest);
                      let newObject = new FileView(self, {
