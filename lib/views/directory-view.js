...
class DirectoryView extends View {
    ...
    let promise = new Promise((resolve, reject) => {
      ...
      if (find) {
        if (find.view().is('.directory')) {
          ...
          self.getRoot().treeView.selectEntry(find.view());
          resolve(true);
        }
      } else {
        if (dir == '' || dir == '/') {
  ...
  newFile(relativePath) {
    ...
    let fullLocalPath = (self.getRoot().getLocalPath(true) + relativePath.replace(/^\//, "").replace(/\/+/g, Path.sep)).replace(/\/+/g, Path.sep);
    ...
    self.getRoot().connector.existsFile(fullRelativePath.trim()).then((result) => {
      self.getRoot().connector.showMessage('File ' + relativePath.trim() + ' already exists', 'error');
    }).catch((err) => {
      return self.getRoot().connector.uploadFile(queueItem).then(() => {
        // Refresh cache
  ...
  newDirectory(relativePath) {
    ...
    self.getRoot().connector.existsDirectory(fullRelativePath.trim()).then((result) => {
      self.getRoot().connector.showMessage('Directory ' + relativePath.trim() + ' already exists', 'error');
    }).catch((err) => {
      return self.getRoot().connector.createDirectory(fullRelativePath.trim()).then((result) => {
        let parentPath = normalize('/' + trailingslashit(dirname(relativePath)));
        let parentObject = null;
  ...
  renameDirectory(relativePath) {
    ...
    self.getRoot().connector.rename(self.getPath(), fullRelativePath.trim()).then(() => {
      // Refresh cache
  ...
  moveDirectory(initialPath, newDirectoryPath) {
    ...
    self.getRoot().connector.rename(src.trim(), dest.trim()).then(() => {
      // Refresh cache
      self.getRoot().getFinderItemsCache().renameDirectory(initialPath, newDirectoryPath);

      // Add new object
      let elementname = basename(dest);
      let pathOnFileSystem = normalize(self.getPath() + elementname, Path.sep);
  ...
  moveFile(initialPath, newDirectoryPath) {
    ...
    self.getRoot().connector.existsFile(dest.trim()).then((result) => {
        ...
        atom.confirm({
          ...
          buttons: {
            Yes: () => {
              ...
              self.getRoot().connector.deleteFile(dest.trim()).then(() => {
                reject(true);
              }).catch((err) => {
                self.getRoot().connector.showMessage(err.message, 'error');
                resolve(false);
  ...
  copyFile(initialPath, newDirectoryPath) {
    ...
    if (initialPath.trim() == newDirectoryPath.trim()) {
      ...
      let parentPath = dirname((((self.config.remote) ? '/' + self.config.remote + '/' : '/') + newDirectoryPath).replace(/\/+/g, "/"));

      self.getRoot().connector.listDirectory(parentPath).then((list) => {
        let files = [];
    ...
    let srcLocalPath = (self.getRoot().getLocalPath(true) + initialPath.replace(/^\//, "").replace(/\/+/g, Path.sep)).replace(/\/+/g, Path.sep);
    let destLocalPath = (self.getRoot().getLocalPath(true) + newDirectoryPath.replace(/^\//, "").replace(/\/+/g, Path.sep)).replace(/\/+/g, Path.sep);
    ...
    self.getRoot().connector.existsFile(dest.trim()).then((result) => {
      ...
      return self.getRoot().connector.downloadFile(download_queueItem).then(() => {
        // Get filesize
        FileSystem.stat(srcLocalPath, (err, stats) => {
          if (stats) {
            ...
            return self.getRoot().connector.uploadFile(upload_queueItem).then(() => {
              // Refresh cache
  ...
  upload(initialPath, newDirectoryPath) {
    ...
    // TODO
    console.log('TODO upload', initialPath, newDirectoryPath);
  }

  deleteDirectory(recursive) {
    const self = this;

    self.getRoot().connector.deleteDirectory(self.getPath(), recursive).then(() => {
      // Refresh cache
