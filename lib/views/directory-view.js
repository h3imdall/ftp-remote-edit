...
class DirectoryView extends View {
  ...
  newFile(relativePath) {
    ...
    self.connector.uploadFile('', fullRelativePath.trim(), null)
      .then(() => {
        if(!dir)dir = '/';
        self.getRoot()
          .expandPath(dir, true)
  ...
  newDirectory(relativePath) {
    ...
    self.connector.createDirectory(fullRelativePath.trim())
      .then((result) => {
        self.getRoot()
          .expandPath(relativePath.trim(), true)
          .catch(function (err) {
            self.connector.showMessage(err.message+err, 'error');
          });
      })
      .catch(function (err) {
        self.connector.showMessage(err.message+err, 'error');
  ...
  rename(relativePath) {
    ...
    self.connector.rename(self.getPath(), fullRelativePath.trim())
      .then(() => {
        let pathArr = relativePath.trim()
          .split('/');
