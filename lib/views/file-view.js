...
class FileView extends View {
  ...
  open(pending = false) {
    ...
    // Download file
    self.getRoot().connector.downloadFile(queueItem).then(() => {
      // Remove sync icon
      self.removeSyncFileIcon();

      // Open file in texteditor
      return atom.workspace.open(self.getLocalPath(true) + self.name, { pending: pending, searchAllPanes: true }).then((editor) => {
  ...
  renameFile(relativePath) {
    ...
    self.getRoot().connector.rename(self.getPath(true) + self.name, fullRelativePath.trim()).then(() => {
      // Refresh cache
  ...
  deleteFile() {
    ...
    self.getRoot().connector.deleteFile(self.getPath(true) + self.name).then(() => {
      // Refresh cache
