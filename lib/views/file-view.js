...
class FileView extends View {
  ...
  open() {
    ...
    self.getRoot().connector.existsFile(self.getPath() + self.name)
      .then((file) => {
        if (file !== undefined && file !== null) {

          // Create local Directories
          self.checkPath(self.getLocalPath() + self.name);

          // Download file and open
          self.getRoot().connector.downloadFile(self.getPath() + self.name, self.getLocalPath() + self.name)
            .then(() => {
            ...
            .then((editor) => {
              if (self.editor === null || self.editor === undefined) {
                ...
                editor.onDidSave((saveObject) => {
                  self.getRoot().connector.uploadFile(editor.getText(), self.getPath() + self.name, editor.getPath())
                    .then(() => {
                      self.getRoot().connector.showMessage('File successfully uploaded.', 'success');
                    })
                    .catch(function (err) {
  ...
  rename(relativePath) {
    ...
    self.getRoot().connector.rename(self.getPath(true) + self.name, fullRelativePath.trim())
      .then(() => {
  ...
  delete() {
    ...
    self.getRoot().connector.deleteFile(self.getPath(true) + self.name)
      .then(() => {
        self.destroy();
      })
      .catch(function (err) {
