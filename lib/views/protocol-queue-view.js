...
class ProtocolQueueView extends ScrollView {
  ...
  initialize(fileinfo) {
    ...
    self.info = {
      ...
      status: (fileinfo.status) ? fileinfo.status : "Wating"
    };
  ...
  changeProgress(data) {
    ...
    self.info.progress += data.length;
  ...
  changeStatus(status) {
    ...
    if (oldStatus == "Transferring" && status.toLowerCase() == "error") {
      self.trigger('protocol-queue-view:error');
...
module.exports = ProtocolQueueView;
