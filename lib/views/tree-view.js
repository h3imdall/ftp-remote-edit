...
class TreeView extends ScrollView {
  ...
  addDirectory(root, relativePath, options = {}) {
    ...
    let directory = self.findElementByPath(elementPath);
    if (!directory) {
      directory = new DirectoryView(root, {
        name: element,
        rights: permissionsToRights(options.rights)
      });
  ...
  addFile(root, relativePath, options = {}) {
    ...
    let file = self.findElementByPath(elementPath);
    if (!file) {
  ...
  expand(root, relativePath) {
    ...
    let promise = new Promise((resolve, reject) => {
      ...
      root.getRoot().expand().then(() => {
        ...
        let dir = untrailingslashit(normalize(root.getPath(false) + arrPath.shift()));
        let newRelativePath = arrPath.join('/');

        let find = self.findElementByPath(dir);
        if (find) {
  ...
  findElementByLocalPath(pathOnFileSystem) {
    ...
    self.servers.forEach((config) => {
      ...
      if (pathOnFileSystem.indexOf(path) != -1) {
        ...
        let find = self.findElementByPath(normalize(unleadingslashit(relativePath), '/'));
        if (find) {
          found = find;
          return;
        }
