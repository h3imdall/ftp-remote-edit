...
import { getConfig, resetIgnoredPatterns } from './../helper/helper.js';
...
class TreeView extends ScrollView {
  ...
  loadServers(password) {
    ...
    let configHash = getConfig('ftp-remote-edit.config');
    if (configHash) {
      ...
      try {
        ...
        let configHashCopy = getConfig('ftp-remote-edit.config');
        let regularExpression = /^[a-zA-Z0-9]+$/
        let detail = {
          config: {
            load: (configHash == configHashCopy),
            decrypt: (config !== null),
            start: (config !== null) ? config.substr(0, 4) : '',
            end: (config !== null) ? config.substr(config.length - 4, 4) : '',
          },
          password: {
        ...
        atom.notifications.addFatalError(e.message, {
          detail: e.message,
          stack: e.stack + '\n\nDetail: ' + b64EncodeUnicode(JSON.stringify(detail)),
          dismissable: true
        });
  ...
  getElementByLocalPath(pathOnFileSystem, root) {
    const self = this;
