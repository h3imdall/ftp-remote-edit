...
import { basename, dirname, trailingslashit, normalize, cleanJsonString } from './../helper/format.js';
...
class TreeView extends ScrollView {
  ...
  initialize(state) {
    ...
    // self.on('focusout', (e) => {
    //   console.log('unfocus',self.forceFocus);
  ...
  findElementByPath(relativePath) {
    const self = this;

    let find = self.list.find('li[id="' + 'ftp-remote-edit-' + md5(relativePath) + '"]');
    if (find.length > 0) {
      return find;
    }

    find = self.list.find('li[id="' + 'ftp-remote-edit-' + md5(relativePath + '/') + '"]');
    if (find.length > 0) {
      return find;
    }
  ...
  findElementByLocalPath(pathOnFileSystem) {
    ...
    pathOnFileSystem = normalize(trailingslashit(pathOnFileSystem), Path.sep);

    if (!self.servers) return;
    if (!self.list) return;
    ...
    self.servers.forEach((config) => {
      ...
      if (pathOnFileSystem.indexOf(path) != -1) {
        ...
        const relativePath = normalize(pathOnFileSystem.replace(normalize(root.getLocalPath(), Path.sep), ''));

        let find = self.findElementByPath('/' + relativePath);
        if (find) {
          found = find.view();
          return;
        }
