...
class TreeView extends ScrollView {
  ...
  initialize(state) {
    ...
    self.on('mousedown', (e) => {
      ...
      if (entry = e.target.closest('.entry')) {
        e.stopPropagation();
        console.log($(entry).view().getPath());
        console.log($(entry).view().getLocalPath());
  ...
  addServer(config) {
    ...
    server.getConnector().on('log', (msg) => {
      self.ftpLogView.trigger('log', msg);
    });
  ...
  expand(root, relativePath) {
    ...
    let promise = new Promise((resolve, reject) => {
      ...
      root.getRoot().expand().then(() => {
        ...
        let dir = normalize(root.getPath(false) + arrPath.shift());
  ...
  findElementByLocalPath(pathOnFileSystem) {
    ...
    self.servers.forEach((config) => {
      ...
      if (pathOnFileSystem.indexOf(path) != -1) {
        ...
        let find = self.findElementByPath(unleadingslashit(relativePath));
        if (find) {
          found = find;
          return;
        }
