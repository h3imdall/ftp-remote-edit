...
class TreeView extends ScrollView {
  ...
  addServer(config) {
    ...
    server.onDidChange = (state) => {
      self.onDidChange(state, server);
    };

    self.list.append(server);
    self.hideInfo();

    self.onDidChange('addServer', server);
  }
  ...
  removeServer(root) {
    ...
    self.onDidChange('removeServer', root);
  }
  ...
  addFolder(config) {
    ...
    folder.onDidChange = (state) => {
      self.onDidChange(state, folder);
    };

    self.list.append(folder);
    self.hideInfo();

    self.onDidChange('addFolder', folder);
  }
  ...
  addDirectory(root, relativePath, options = {}) {
    ...
    if (!directory) {
      ...
      directory.onDidChange = (state) => {
        self.onDidChange(state, directory);
      };
    ...
    if (tmp.length > 0) {
      ...
      self.onDidChange('addDirectory', directory);
  ...
  addFile(root, relativePath, options = {}) {
    ...
    if (!file) {
      ...
      file.onDidChange = (state) => {
        self.onDidChange(state, file);
      };
