...
export default class ConfigurationView extends ScrollView {
  ...
  constructor() {
    ...
    let html = '<p>FTP Server Configuration</p>';
    html += "<p>Now you can edit your connection settings. With the checkbox you can toggle between ftp and sftp(ssh).</p>";
  ...
  createRequiredDiv() {
    ...
    self.nameInput.getModel()
      .onDidChange(() => {
        if (self.selectServer.selectedOptions[0]) {
    ...
    self.passwordInput.getModel()
      .onDidChange(() => {
        if (self.selectServer.selectedOptions[0]) {
          let index = self.selectServer.selectedOptions[0].value;
          self.configArray[index].password = self.passwordInput.getText().trim();
        }
      });
  ...
  reload() {
    ...
    if (configText) {
      try {
        ...
        configText = secure.decrypt(self.password, configText);
      }
  ...
  save() {
    ...
    let secure = new Secure();
    if(self.configArray.length > 0){
      atom.config.set('ftp-remote-edit.config', secure.encrypt(self.password, JSON.stringify(self.configArray)));
    }else{
  ...
  delete() {
    ...
    if (self.configArray.length === 0) return false;
    self.nameInput.setText('')
    self.hostInput.setText('');
    self.portInput.setText('');
    self.userInput.setText('');
    ...
    self.selectServer.selectedOptions[0].remove();
    ...
    removeAllOptionsFromSelect(self.selectServer);

    self.configArray = self.configArray
      .sort((a, b) => {
        if (a.host < b.host) return -1;
        if (a.host > b.host) return 1;
        return 0;
    ...
    self.selectServer.dispatchEvent(new Event('change'));
  }
}
