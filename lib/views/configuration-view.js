...
export default class ConfigurationView extends ScrollView {
  ...
  createRequiredDiv() {
    ...
    passwordLabelTitle.textContent = `Password for authentication.`;
    ...
    remoteControl.classList.add('password');
  ...
  reload(loadConfig, selectedServer) {
    ...
    if (self.configArray.length !== 0) {
      ...
      self.nameInput.setText(self.configArray[selectedIndex].name ? self.configArray[selectedIndex].name : self.configArray[indexInArray].host);
