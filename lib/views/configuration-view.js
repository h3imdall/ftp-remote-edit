...
export default class ConfigurationView extends View {
  ...
  createRequiredDiv() {
    ...
    agentLabel.classList.add('control-label');
    let agentTitle = document.createElement('div');
    agentTitle.classList.add('setting-title')
    agentTitle.textContent = 'Use ssh-agent for authentication.';
    self.agentInput = document.createElement('input');
    self.agentInput.type = 'checkbox';
    self.agentInput.checked = true;
    self.agentInput.classList.add('input-checkbox');
    ...
    let agentControl = document.createElement('div');
    agentControl.classList.add('controls');
    agentControl.classList.add('protocol');
    agentControl.classList.add('checkbox');
    agentControl.appendChild(agentLabel);
    agentLabel.appendChild(self.agentInput);
    ...
    credentialGroup.appendChild(agentControl);
    credentialGroup.appendChild(self.passwordControl);
    divRequired.appendChild(credentialGroup);

    self.protocolControl = document.createElement('div');
    self.protocolControl.classList.add('controls');
    self.protocolControl.classList.add('protocol');
    self.protocolControl.classList.add('checkbox');
    self.protocolControl.appendChild(protocolLabel);
    ...
    protocolGroup.appendChild(self.protocolControl);
    ...
    self.nameInput.getModel().onDidChange(() => {
      if (self.configArray.length !== 0 && !self.disableEventhandler) {
        ...
        self.configArray[index].name = self.nameInput.getText().trim();
    ...
    self.hostInput.getModel().onDidChange(() => {
      if (self.configArray.length !== 0 && !self.disableEventhandler) {
        let index = self.selectServer.selectedOptions[0].value;
        self.configArray[index].host = self.hostInput.getText().trim();
      }
    });
    self.portInput.getModel().onDidChange(() => {
      if (self.configArray.length !== 0 && !self.disableEventhandler) {
        let index = self.selectServer.selectedOptions[0].value;
        self.configArray[index].port = self.portInput.getText().trim();
      }
    });
    self.userInput.getModel().onDidChange(() => {
      if (self.configArray.length !== 0 && !self.disableEventhandler) {
        let index = self.selectServer.selectedOptions[0].value;
        self.configArray[index].user = self.userInput.getText().trim();
      }
    });
    ...
    passwordModel.buffer.onDidChange((obj) => {
      if (!changing) {
        ...
        if (self.configArray.length !== 0 && !self.disableEventhandler) {
          let index = self.selectServer.selectedOptions[0].value;
          self.configArray[index].password = passwordModel.clearTextPassword.getText().trim();
        }
    ...
    self.agentInput.addEventListener('change', (event) => {
      if (self.configArray.length !== 0 && !self.disableEventhandler) {
        let index = self.selectServer.selectedOptions[0].value;
        self.configArray[index].useAgent = self.agentInput.checked;

        if (self.agentInput.checked) {
          self.privatekeyfileControl.setAttribute("style", "display:none;");
          self.passwordControl.setAttribute("style", "display:none;");
          self.protocolControl.setAttribute("style", "display:none;");
        } else {
          self.privatekeyfileControl.removeAttribute("style");
          self.passwordControl.removeAttribute("style");
          self.protocolControl.removeAttribute("style");
        }
      }
    });
    self.privatekeyfileInput.getModel().onDidChange(() => {
      if (self.configArray.length !== 0 && !self.disableEventhandler) {
        let index = self.selectServer.selectedOptions[0].value;
        self.configArray[index].privatekeyfile = self.privatekeyfileInput.getText().trim();
      }
    });
    self.remoteInput.getModel().onDidChange(() => {
      if (self.configArray.length !== 0 && !self.disableEventhandler) {
        let index = self.selectServer.selectedOptions[0].value;
        self.configArray[index].remote = self.remoteInput.getText().trim();
      }
    });
  ...
  createServerSelect() {
    ...
    select.addEventListener('change', (event) => {
      if (self.configArray.length !== 0 && !self.disableEventhandler) {
        ...
        self.agentInput.checked = self.configArray[indexInArray].useAgent;
  ...
  reload(loadConfig, selectedServer) {
    ...
    if (self.configArray.length !== 0) {
      ...
      self.agentInput.checked = self.configArray[selectedIndex].useAgent;
