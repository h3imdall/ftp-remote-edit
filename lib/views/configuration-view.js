...
export default class ConfigurationView extends View {
  ...
  createControls() {
    ...
    serverGroup.appendChild(protocolControl);
    serverGroup.appendChild(logonTypeControl);
    ...
    self.protocolSelect.addEventListener('change', (event) => {
      if (self.configArray.length !== 0 && !self.disableEventhandler) {
        ...
        if (option.value == 'sftp') {
          self.configArray[index].sftp = true;
        } else {
          self.configArray[index].logon = 'credentials';
          self.configArray[index].sftp = false;
          self.configArray[index].useAgent = false;
          self.configArray[index].privatekeyfile = '';
        }
        self.fillInputFields(self.configArray[index]);
      }
    });
    ...
    self.logonTypeSelect.addEventListener('change', (event) => {
      if (self.configArray.length !== 0 && !self.disableEventhandler) {
        ...
        if (option.value == 'credentials') {
          ...
          self.configArray[index].logon = 'keyfile';
          self.configArray[index].useAgent = false;
        } else if (option.value == 'agent') {
          self.configArray[index].logon = 'agent';
          self.configArray[index].useAgent = true;
          self.configArray[index].privatekeyfile = '';
          self.configArray[index].password = '';
        } else {
          self.configArray[index].useAgent = false;
        }
        self.fillInputFields(self.configArray[index]);
  ...
  createServerSelect() {
    ...
    select.addEventListener('change', (event) => {
      if (self.configArray.length !== 0 && !self.disableEventhandler) {
        ...
        self.fillInputFields((indexInArray) ? self.configArray[indexInArray] : null);
      }
    });
  ...
  reload(loadConfig, selectedServer) {
    ...
    if (self.configArray.length !== 0) {
      ...
      self.fillInputFields((selectedIndex) ? self.configArray[selectedIndex] : null);
  ...
  test() {
    ...
    let config = this.configArray[indexInArray];
  ...
  editFolders() {
    ...
    // TODO create folderConfigurationView
    // const folderConfigurationView = new FolderConfigurationView('', true);
    // folderConfigurationView.on('confirm', (e, relativePath) => {
    //   // Close folder configuration
    //   folderConfigurationView.close(); 

    //   // Show configuration
    //   this.panel.show();
    // });

    // folderConfigurationView.on('cancel', () => {
    //   // Show configuration
    //   this.panel.show();
    // });
