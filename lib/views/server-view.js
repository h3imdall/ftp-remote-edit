...
class ServerView extends View {
  ...
  promptPassword() {
    ...
    let promise = new Promise((resolve, reject) => {
      dialog.on('dialog-done', (e, password) => {
        self.config.passwordDecrypted = password;
        dialog.close();
        resolve(true);
      });
      dialog.attach();
  ...
  expand() {
    ...
    let promise = new Promise((resolve, reject) => {
      ...
      if(typeof self.config.passwordDecrypted === 'undefined') {
        if (self.config.logon == 'passwordprompt' && !self.config.password) {
          self.promptPassword().then(() => {
            self.expand().then(() => {}, () => {
              self.config.passwordDecrypted = "";
            });
        });
        elseif (atom.config.get('ftp-remote-edit.encryption') == 'encrypted') {
              self.config.passwordDecrypted = decrypt(Storage.getPassword(), self.config.password);
        } else {
              self.config.passwordDecrypted = self.config.password;
        }
