...
import { showMessage, highlight } from './../helper/helper.js';
...
export default class FinderView extends EventEmitter {
  ...
  open(item) {
    ...
    try {
      ...
      let find = self.treeView.list.find('#' + file.getRoot().getId());

      if (!find) return;

      let root = find.view().getRoot();
      self.treeView.expand(root, relativePath).then(() => {
