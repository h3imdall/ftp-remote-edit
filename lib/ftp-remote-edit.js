...
class FtpRemoteEdit {
  ...
  paste() {
    ...
    if (JSON.stringify(destObject.config) == JSON.stringify(srcObject.config)) {
      ...
      if (handleEvent == "cut") {
        if (srcObject.is('.directory')) destObject.moveDirectory(srcPathToCopy, destPathToCopy);
        if (srcObject.is('.file')) destObject.moveFile(srcPathToCopy, destPathToCopy);
      } else if (handleEvent == "copy") {
        if (srcObject.is('.directory')) destObject.copyDirectory(srcPathToCopy, destPathToCopy);
        if (srcObject.is('.file')) destObject.copyFile(srcPathToCopy, destPathToCopy);
      }
    } else {
