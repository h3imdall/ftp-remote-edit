...
import FinderDialog from './dialogs/finder-dialog.js';
...
class FtpRemoteEdit {
  ...
  activate() {
    ...
    self.listeners.add(atom.commands.add('atom-workspace', {
      ...
      'ftp-remote-edit:finder-refresh': () => self.refreshFinder(),
  ...
  remotePathFinder() {
    const self = this;
    const dialog = new FinderDialog();
    const selected = self.treeView.list.find('.selected');
    let root = null;
    root = selected.view()
        .getRoot();
    //now, only support sftp
    if(!root.config.sftp) {
      return;
    }
    var pathsParamName = root.config.remote + 'paths';
    if(!self[pathsParamName]) {
      root.connector.listRemotePaths(root.config.remote + '/')
      .then((result) => {
        self[pathsParamName] = result;
        selected.view().click();
        dialog.toggle(self[pathsParamName], selected);
      });
    } else {
      dialog.toggle(self[pathsParamName], selected);
    }
