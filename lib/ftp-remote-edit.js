  ...
  uploadDirectory(server, srcPath, destPath) {
    ...
    let promise = new Promise((resolve, reject) => {
      FileSystem.list(srcPath, (err, result) => {
        ...
        return resolve(Promise.all(result.map((path) => {
          if (FileSystem.statSync(path).isDirectory()) {
            return self.uploadDirectory(server, path, normalize(destPath + '/' + basename(path, Path.sep), '/')).catch((err) => { console.log(err); });
          } else {
            return self.uploadFile(server, path, normalize(destPath + '/' + basename(path, Path.sep), '/')).catch((err) => { console.log(err); });
          }
        })));
      });
    });
    ...
    return server.getConnector().listDirectory(srcPath).then((list) => {
      return Promise.all(list.map((item) => {
        ...
        if (item.type === 'd' && item.name !== '.' && item.name !== '..') {
          ...
          return self.downloadDirectory(server, srcItemPath, destItemPath).catch((err) => { console.log(err); });
        } else if (item.type === '-') {
          return self.downloadFile(server, srcItemPath, destItemPath).catch((err) => { console.log(err); });
        }
      }));
    }).catch((err) => {
      return Promise.reject(err);
