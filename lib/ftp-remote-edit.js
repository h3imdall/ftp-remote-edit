...
import { showMessage, getFullExtension, createLocalPath, deleteLocalPath, getTextEditor, permissionsToRights } from './helper/helper.js';
...
class FtpRemoteEdit {
  ...
  getCurrentServerConfig() {
    ...
    return (reasonForRequest) => {
      return new Promise((resolve, reject) => {
        ...
        if (isInBlackList(self.info.password, reasonForRequest)) {
          reject('userdeclined');
          return;
        }
        if (isInWhiteList(self.info.password, reasonForRequest)) {
          resolve(root.config);
          return;
        }
        ...
        let notif = atom.notifications.addWarning('Server Configuration Requested', {
          ...
          buttons: [{
              ...
              onDidClick: () => {
                buttondismiss = true;
                notif.dismiss();
                addToWhiteList(self.info.password, reasonForRequest);
                resolve(root.config);
              }
            },
            {
            ...
            },
            {
              ...
              onDidClick: () => {
                buttondismiss = true;
                notif.dismiss();
                addToBlackList(self.info.password, reasonForRequest);
                reject('userdeclined');
              }
            },
          ]
  ...
  createFile(directory, relativePath) {
    ...
    directory.getRoot().connector.existsFile(fullRelativePath).then((result) => {
      ...
      return directory.getRoot().connector.uploadFile(queueItem).then(() => {
        // Refresh cache
        directory.getRoot().getFinderCache().addFile(relativePath, 0);
        ...
        if (parentPath == '/') {
          ...
          parentObject = self.treeView.findElementByPath(parentPath);
        }
  ...
  createDirectory(directory, relativePath) {
    ...
    directory.getRoot().connector.existsDirectory(fullRelativePath).then((result) => {
      showMessage('Directory ' + relativePath.trim() + ' already exists', 'error');
    }).catch((err) => {
      return directory.getRoot().connector.createDirectory(fullRelativePath).then((result) => {
        let parentPath = normalize('/' + trailingslashit(dirname(relativePath)));
        let parentObject = null;
  ...
  renameFile(file, relativePath) {
    ...
    file.getRoot().connector.rename(file.getPath(true) + file.name, fullRelativePath).then(() => {
      // Refresh cache
      file.getRoot().getFinderCache().renameFile(file.getPath(false) + file.name, relativePath, file.size);

      // get info from old object
      let oldObject = file;

      // Add new object
      let parentPath = ('/' + trailingslashit(dirname(normalize(relativePath))));
      let parentObject = null;
      ...
      if (parentObject) {
        ...
        let newObject = self.treeView.getElementByLocalPath(fullLocalPath, file.getRoot(), 'file');
  ...
  renameDirectory(directory, relativePath) {
    ...
    directory.getRoot().connector.rename(directory.getPath(), fullRelativePath).then(() => {
      // Refresh cache
      directory.getRoot().getFinderCache().renameDirectory(directory.getPath(false), relativePath + '/');
      ...
      // Move local directoy
      let initialPath = oldObject.getLocalPath(true);
      let arrPath = fullLocalPath.split(Path.sep);
      arrPath.pop();
      let newDirectoryPath = arrPath.join(Path.sep);
      let newPath = fullLocalPath;
  ...
  deleteFile(file) {
    ...
    file.getRoot().connector.deleteFile(file.getPath(true) + file.name).then(() => {
      // Refresh cache
      file.getRoot().getFinderCache().deleteFile(file.getPath(false) + file.name);
  ...
  deleteDirectory(directory, recursive) {
    ...
    directory.getRoot().connector.deleteDirectory(directory.getPath(), recursive).then(() => {
      // Refresh cache
      directory.getRoot().getFinderCache().deleteDirectory(directory.getPath(false));
      ...
      if (initialPath) {
        ...
        for (let i = 0, len = ref.length; i < len; i++) {
          ...
          if (FileSystem.statSync(file.path).isDirectory()) {
            self.uploadDirectory(file.path, newDirectoryPath + basename(file.path, Path.sep));
          } else {
            self.uploadFile(file.path, newDirectoryPath + basename(file.path, Path.sep));
          }
        }
      }
    }
  ...
  moveFile(directory, initialPath, newDirectoryPath) {
    ...
    // get info from old object
    let oldObject = self.treeView.findElementByPath(trailingslashit(initialPath));
    let fileinfo = {
      size: 0,
      rights: null,
      content: "",
    ...
    directory.getRoot().connector.existsFile(dest).then((result) => {
      return new Promise((resolve, reject) => {
        atom.confirm({
          ...
          buttons: {
            Yes: () => {
              ...
              directory.getRoot().connector.deleteFile(dest).then(() => {
                reject(true);
              }).catch((err) => {
                showMessage(err.message, 'error');
                resolve(false);
      ...
      // add sync icon
      if (oldObject) {
        oldObject.addSyncIcon();
      }

      directory.getRoot().connector.rename(src, dest).then(() => {
        // Refresh cache
        directory.getRoot().getFinderCache().renameFile(src.replace(directory.getRoot().config.remote, ''), dest.replace(directory.getRoot().config.remote, ''), fileinfo.size);
  ...
  moveDirectory(directory, initialPath, newDirectoryPath) {
    ...
    directory.getRoot().connector.existsDirectory(dest).then((result) => {
      return new Promise((resolve, reject) => {
        atom.confirm({
          ...
          buttons: {
            Yes: () => {
              dirinfo.exists = true;
              directory.getRoot().connector.deleteDirectory(dest, recursive).then(() => {
                reject(true);
              }).catch((err) => {
                showMessage(err.message, 'error');
                resolve(false);
      ...
      directory.getRoot().connector.rename(src, dest).then(() => {
        // Refresh cache
        directory.getRoot().getFinderCache().renameDirectory(initialPath, newDirectoryPath);
        ...
        arrPath.pop();
        let newDirectoryPath = arrPath.join(Path.sep);
        let newPath = newObject.getLocalPath(true);
  ...
  copyFile(directory, initialPath, newDirectoryPath) {
    ...
    if (normalize(initialPath) == normalize(newDirectoryPath)) {
      ...
      let parentPath = normalize('/' + directory.config.remote + '/' + newDirectoryPath);
    ...
    directory.getRoot().connector.existsFile(dest).then((result) => {
      ...
      return directory.getRoot().connector.downloadFile(downloadQueueItem).then(() => {
        // Get filesize
        FileSystem.stat(srcLocalPath, (err, stats) => {
          if (stats) {
            ...
            return directory.getRoot().connector.uploadFile(uploadQueueItem).then(() => {
              // Refresh cache
              directory.getRoot().getFinderCache().addFile(dest.replace(directory.getRoot().config.remote, ''), directory.size);
  ...
  remotePathFinder(reindex = false) {
    ...
    if (self.finderView == null) {
      ...
      self.finderView.on('ftp-remote-edit-finder:open', (file) => {
        self.openFile(file);
      });
    }
