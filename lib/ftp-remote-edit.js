...
class FtpRemoteEdit {
  ...
  openFile(file, pending = false) {
    ...
    self.downloadFile(file, file.getLocalPath(true) + file.name).then(() => {
  ...
  duplicateFile(file, relativePath) {
    ...
    file.getConnector().existsFile(fullRelativePath).then(() => {
      ...
      self.downloadFile(file, fullLocalPath).then(() => {
        self.uploadFile(file.parent, fullLocalPath, relativePath).then((duplicatedFile) => {
          if (duplicatedFile) {
            // Open file and add handler to editor to upload file on save
      ...
      if (initialPath) {
        ...
        for (let i = 0, len = ref.length; i < len; i++) {
          ...
          if (FileSystem.statSync(file.path).isDirectory()) {
            self.uploadDirectory(destObject, file.path, newDirectoryPath + basename(file.path, Path.sep)).catch((err) => { });
          } else {
            self.uploadFile(destObject, file.path, newDirectoryPath + basename(file.path, Path.sep)).catch((err) => { });
          }
        }
      }
    }
      ...
      Electron.remote.dialog.showOpenDialog(null, { title: 'Select file(s)= for upload...', defaultPath: Electron.remote.app.getPath("desktop"), buttonLabel: 'Upload', properties: ['openFile', 'multiSelections', 'showHiddenFiles'] }, (filePaths, bookmarks) => {
        if (filePaths) {
          filePaths.forEach((filePath, index) => {
            srcPath = filePath;
            destPath = destObject.getPath(false) + basename(filePath, Path.sep);

            self.uploadFile(destObject, srcPath, destPath).catch((err) => { });
          });
        }
      });
    } else if (type == 'directory') {
      Electron.remote.dialog.showOpenDialog(null, { title: 'Select directory for upload...', defaultPath: Electron.remote.app.getPath("desktop"), buttonLabel: 'Upload', properties: ['openDirectory', 'showHiddenFiles'] }, (directoryPaths, bookmarks) => {
        if (directoryPaths) {
          directoryPaths.forEach((directoryPath, index) => {
            ...
            destPath = destObject.getPath(false) + basename(directoryPath, Path.sep);

            self.uploadDirectory(destObject, srcPath, destPath).catch((err) => { });
          });
        }
      });
    }
    ...
    if (selected.view().is('.file')) {
      ...
      if (file) {
        Electron.remote.dialog.showSaveDialog(null, { defaultPath: Electron.remote.app.getPath("downloads") + "/" + file.name }, (destPath) => {
          if (destPath) {
            self.downloadFile(file, destPath).then(() => {
              showMessage('File has been saved to ' + destPath, 'success');
            }).catch((err) => { });
          }
        });
      }
      ...
      if (directory) {
        Electron.remote.dialog.showSaveDialog(null, { defaultPath: Electron.remote.app.getPath("downloads") + "/" + directory.name }, (destPath) => {
          if (destPath) {
            self.downloadDirectory(directory, destPath);
          }
        });
      }
      ...
      if (server) {
        Electron.remote.dialog.showSaveDialog(null, { defaultPath: Electron.remote.app.getPath("downloads") + "/" }, (destPath) => {
          if (destPath) {
            self.downloadDirectory(server, destPath);
          }
        });
      }
    }
      ...
      FileSystem.list(initialPath, (err, result) => {
        result.forEach((path, index) => {
          if (FileSystem.statSync(path).isDirectory()) {
            resolve(self.uploadDirectory(directory, path, normalize(newDirectoryPath + '/' + basename(path, Path.sep), '/')).catch((err) => { reject(err); }));
          } else {
            resolve(self.uploadFile(directory, path, normalize(newDirectoryPath + '/' + basename(path, Path.sep), '/')).catch((err) => { reject(err); }));
          }
        })
      });
    });
  ...
  downloadDirectory(directory, destPath) {
    // Add sync icon
    directory.addSyncIcon();

    if (!FileSystem.existsSync(destPath)) {
      FileSystem.mkdir(destPath);
    }

    let connector = directory.getConnector();
    let remote_path = directory.getPath();

    let downloadDirectory = function (remote_path, local_path) {
      connector.listDirectory(remote_path).then((list) => {
        list.forEach((item, index) => {
          let new_local_path = local_path + "/" + item.name;
          let new_remote_path = remote_path + "/" + item.name;
          if (item.type === 'd' && item.name !== '.' && item.name !== '..') {
            if (!FileSystem.existsSync(new_local_path)) {
              FileSystem.mkdir(new_local_path);
            }
            downloadDirectory(new_remote_path, new_local_path);
          } else if (item.type === '-') {
            // Add to Download Queue
            let queueItem = Queue.addFile({
              direction: "download",
            ...
            connector.downloadFile(queueItem).then(() => { }).catch((err) => {
              queueItem.changeStatus('Error');
              showMessage(err, 'error');
            });
          }
    ...
    downloadDirectory(remote_path, destPath);

    // Remove sync icon
    directory.removeSyncIcon();
  }
