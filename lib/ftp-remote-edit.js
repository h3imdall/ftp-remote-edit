...
class FtpRemoteEdit {
  ...
  remotePathFinder() {
    ...
    itemsCache.on('finder-items-cache-queue:update', (items) => {
      pathsFound = items.length
      self.finderView.selectListView.update({ items: items, errorMessage: '', loadingMessage: 'Indexing\u2026' + pathsFound })
    });
    itemsCache.on('finder-items-cache-queue:finish', () => {
      self.finderView.selectListView.update({ errorMessage: '', loadingMessage: ''})
    });
    itemsCache.on('finder-items-cache-queue:error', (err) => {
