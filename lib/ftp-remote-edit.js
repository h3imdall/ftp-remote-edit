...
class FtpRemoteEdit {
    ...
    return atom.workspace.open(normalize(file.getLocalPath(true) + file.name, Path.sep), { pending: pending, searchAllPanes: true }).then((editor) => {
      ...
      try {
        ...
        editor.onDidDestroy(() => {
          ...
          editor.saveObject.onDidChange('close');
  ...
  getTreeViewInstance() {
    ...
    if (self.treeView == null) {
      ...
      self.treeView.onDidChange = (state, item) => {
        console.log(state);
        // if (self.loaded && Storage.loaded) {
        //   Storage.state.treeView = self.getTreeViewInstance().serialize();
        //   Storage.saveState();
        // }
      };
    }
  ...
  getProtocolViewInstance() {
    ...
    if (self.protocolView == null) {
      ...
      self.protocolView.onDidChange = (state, item) => {
        console.log(state);
        // if (self.loaded && Storage.loaded) {
        //   Storage.state.protocolView = self.getProtocolViewInstance().serialize();
        //   Storage.saveState();
        // }
      };
    }
