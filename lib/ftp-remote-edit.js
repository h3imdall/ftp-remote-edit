...
class FtpRemoteEdit {
  ...
  activate() {
    ...
    self.subscriptions.add(atom.commands.add('atom-workspace', {
      ...
      'ftp-remote-edit:upload-files': () => self.uploadFiles(),
    }));
  ...
  uploadFiles() {
      ...
      const selected = self.treeView.list.find('.selected');
      if (selected.length === 0) return;
      if (!(selected.view().is('.directory') || selected.view().is('.server'))) return;

      let directory = selected.view();

      Electron.remote.dialog.showOpenDialog(null, { defaultPath: Electron.remote.app.getPath("desktop"), buttonLabel: 'Upload files', properties: ['multiSelections', 'openFile']}, (paths, bookmarks) => {
          paths.forEach((item, index) => {
              let fileinfo = Fs.statSync(item);
              let remotePath = directory.getPath(false) + Path.basename(item);

              directory.getConnector().existsFile(remotePath).then((result) => {
                showMessage('File ' + remotePath.trim() + ' already exists', 'error');
              }).catch((err) => {
                  let queueItem = Queue.addFile({
                    direction: "upload",
                  ...
                  directory.getConnector().uploadFile(queueItem).then(() => {
                      self.treeView.addFile(directory.getRoot(), remotePath);
                  }).catch((err) => {
                    queueItem.changeStatus('Error');
