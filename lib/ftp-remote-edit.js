...
class FtpRemoteEdit {
  ...
  downloadFile(file) {
    Electron.remote.dialog.showSaveDialog(null, { defaultPath: Electron.remote.app.getPath("downloads") + "/" + file.name }, (path) => {
      // Check if file is already in Queue
      if (Queue.existsFile(path)) {
        return false;
      }
      ...
      file.getConnector().downloadFile(queueItem).then(() => {
        // Remove sync icon
        file.removeSyncIcon();
        showMessage('File has been saved!', 'success');
        return true;
      }).catch((err) => {
  ...
  downloadDirectory(directory) {
    Electron.remote.dialog.showSaveDialog(null, { defaultPath: Electron.remote.app.getPath("downloads") + "/" + directory.name }, (path) => {
      // Add sync icon
      ...
      downloadDirectory(remote_path, path);
      ...
      Electron.remote.dialog.showOpenDialog(null, { title: 'Select file(s)= for upload...', defaultPath: Electron.remote.app.getPath("desktop"), buttonLabel: 'Upload', properties: ['openFile', 'multiSelections', 'showHiddenFiles'] }, (filePaths, bookmarks) => {
        if (filePaths) {
          filePaths.forEach((filePath, index) => {
            ...
            self.uploadFile(destObject, srcPath, destPath)
          });
        }
      });
    } else if (type == 'directory') {
      Electron.remote.dialog.showOpenDialog(null, { title: 'Select directory for upload...', defaultPath: Electron.remote.app.getPath("desktop"), buttonLabel: 'Upload', properties: ['openDirectory', 'showHiddenFiles'] }, (directoryPaths, bookmarks) => {
        if (directoryPaths) {
          directoryPaths.forEach((directoryPath, index) => {
            ...
            self.uploadDirectory(destObject, srcPath, destPath)
          });
        }
      });
    }
