...
class FtpRemoteEdit {
  ...
  activate() {
    ...
    self.subscriptions.add(atom.commands.add('atom-workspace', {
      'ftp-remote-edit:show': () => this.show(),
      'ftp-remote-edit:toggle': () => self.toggle(),
      'ftp-remote-edit:hide': () => this.treeView.hide(),
    ...
    file.getConnector().existsFile(fullRelativePath).then(() => {
      ...
      file.getConnector().downloadFile(downloadQueueItem).then(() => {
        ...
        file.getConnector().uploadFile(uploadQueueItem).then(() => {
          ...
          if (parentObject) {
            let elementname = basename(fullRelativePath);
            let pathOnFileSystem = fullLocalPath;
            let newObject = new FileView(parentObject, {
              name: elementname,
              path: pathOnFileSystem,
              size: file.size,
  ...
  moveFile(directory, initialPath, newDirectoryPath) {
    ...
    directory.getConnector().existsFile(dest).then((result) => {
      ...
      directory.getConnector().rename(src, dest).then(() => {
        // Refresh cache
        directory.getRoot().getFinderCache().renameFile(normalize(src.replace(directory.getRoot().config.remote, '')), normalize(dest.replace(directory.getRoot().config.remote, '')), fileinfo.size);
  ...
  copyFile(directory, initialPath, newDirectoryPath) {
    const self = this;
    console.log(directory, initialPath, newDirectoryPath);
