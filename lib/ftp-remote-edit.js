  ...
  duplicateFile(file, relativePath) {
    ...
    file.getConnector().existsFile(fullRelativePath).then(() => {
      ...
      file.getConnector().downloadFile(downloadQueueItem).then(() => {
        ...
        FileSystem.copyFileSync(file.getLocalPath(true) + file.name, fullLocalPath);
        ...
        file.getConnector().uploadFile(uploadQueueItem).then(() => {
          // Refresh cache
          file.getRoot().getFinderCache().addFile(relativePath, 0);
          ...
          if (parentObject) {
            ...
            // Open file and add handler to editor to upload file on save
            return self.openFileInEditor(newObject);
          } else {
            let newObject = self.treeView.getElementByLocalPath(fullLocalPath, file.getRoot(), 'file');
