...
class FtpRemoteEdit {
  ...
  activate() {
    ...
    self.subscriptions.add(atom.commands.add('atom-workspace', {
      ...
      'ftp-remote-edit:reopen-file': () => self.open(false,true),
  ...
  open(pending = false, reload = false) {
    ...
    if (selected.view().is('.file')) {
      ...
      if (file) {
        self.openFile(file, pending, reload);
      }
  ...
  openFile(file, pending = false, reload = false) {
    const self = this;
    ...
    if (getTextEditor(file.getLocalPath(true) + file.name, true)) {
      if(reload) {
          getTextEditor(file.getLocalPath(true) + file.name, true).destroy();
      } else {
