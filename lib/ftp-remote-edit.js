...
class FtpRemoteEdit {
  ...
  openFile(file, pending = false) {
    ...
    self.downloadFile(file.getRoot(), fullRelativePath, fullLocalPath).then(() => {
  ...
  duplicateFile(file, relativePath) {
    ...
    file.getConnector().existsFile(fullRelativePath).then(() => {
      ...
      self.downloadFile(file.getRoot(), file.getRoot().getPath(true) + file.name, fullLocalPath).then(() => {
  ...
  reloadFile(file) {
    ...
    // Check if file is already opened in texteditor
    if (getTextEditor(fullLocalPath, true)) {
      self.downloadFile(file.getRoot(), fullRelativePath, fullLocalPath).then(() => { }).catch((err) => { });
    }
  }
  ...
  paste() {
    ...
    if (handleEvent == "cut") {
      ...
      if (srcType == 'file') self.copyFile(destObject.getRoot(), srcPath, destPath);
    }
  }
    ...
    if (selected.view().is('.file')) {
      ...
      if (file) {
        ...
        Electron.remote.dialog.showSaveDialog(null, { defaultPath: Electron.remote.app.getPath("downloads") + "/" + file.name }, (destPath) => {
          if (destPath) {
            self.downloadFile(file.getRoot(), srcPath, destPath).then(() => {
      ...
      if (directory) {
        ...
        Electron.remote.dialog.showSaveDialog(null, { defaultPath: Electron.remote.app.getPath("downloads") + "/" + directory.name }, (destPath) => {
          if (destPath) {
            self.downloadDirectory(directory.getRoot(), srcPath, destPath).catch((err) => { });
          }
        });
      }
      ...
      if (server) {
        ...
        Electron.remote.dialog.showSaveDialog(null, { defaultPath: Electron.remote.app.getPath("downloads") + "/" }, (destPath) => {
          if (destPath) {
            self.downloadDirectory(server, srcPath, destPath).catch((err) => { });
          }
        });
      }
    }
  ...
  copyFile(server, srcPath, destPath) {
    const self = this;
    ...
    server.getConnector().existsFile(destPath).then((result) => {
      ...
      self.downloadFile(server, srcPath, destLocalPath).then(() => {
  ...
  uploadDirectory(server, srcPath, destPath) {
    ...
    let promise = new Promise((resolve, reject) => {
      FileSystem.list(srcPath, (err, result) => {
        result.forEach((path, index) => {
          if (FileSystem.statSync(path).isDirectory()) {
            resolve(self.uploadDirectory(server, path, normalize(destPath + '/' + basename(path, Path.sep), '/')).catch((err) => { reject(err); }));
          } else {
            resolve(self.uploadFile(server, path, normalize(destPath + '/' + basename(path, Path.sep), '/')).catch((err) => { reject(err); }));
          }
        })
      });
    });
  ...
  downloadFile(server, srcPath, destPath) {
    const self = this;
  ...
  downloadDirectory(server, srcPath, destPath) {
    ...
    let promise = new Promise((resolve, reject) => {
      ...
      return server.getConnector().listDirectory(srcPath).then((list) => {
        list.forEach((item, index) => {
          let destItemPath = normalize(destPath + Path.sep + item.name, Path.sep);
          let srcItemPath = normalize(srcPath + "/" + item.name);
          if (item.type === 'd' && item.name !== '.' && item.name !== '..') {
            if (!FileSystem.existsSync(destItemPath)) {
              FileSystem.mkdirSync(destItemPath);
            }
            resolve(self.downloadDirectory(server, srcItemPath, destItemPath).catch((err) => { reject(err); }));
          } else if (item.type === '-') {
            resolve(self.downloadFile(server, srcItemPath, destItemPath).catch((err) => { reject(err); }));
          }
        });
      }).catch((err) => {
        reject(err);
